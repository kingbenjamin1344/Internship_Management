...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'create_job') {
    ...
    if (!$company || $title === '') {
        ...
        $companyNameStmt = $pdo->prepare('SELECT company_name FROM companies WHERE id = ?');
        $companyNameStmt->execute([$companyId]);
        $companyNameResult = $companyNameStmt->fetch();
        $companyName = $companyNameResult ? $companyNameResult['company_name'] : 'Unknown Company';
        ...
        notifyAdmins(
            ...
            'new_job',
            'New Job Posted',
            "New job '{$title}' at {$companyName} has been posted.",
            'company.php'
        );
